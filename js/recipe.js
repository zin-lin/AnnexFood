...
const recipeLoad = document.getElementById('recipe-load');
const loadCon = document.getElementById('load-con');
...
if (did){
    ...
    doc.get().then((d) => {
        if (d !== null) {
            recipeLoad.style.visibility = `hidden`;
            recipeLoad.style.opacity = `0`;
            recipeLoad.style.height = 0;
            loadCon.style.visibility = `visible`;
            loadCon.style.opacity = `1`;
            ...
            var mainRep = document.getElementById("mainRep");
            mainRep.innerHTML = `
    <p> Sorry, Nae recipe exist or cannot find due to network errors</p>
    `
        }
    }).catch(err => {
        var mainRep = document.getElementById("mainRep");
